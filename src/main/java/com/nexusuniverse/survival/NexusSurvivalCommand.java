...
public class NexusSurvivalCommand implements CommandExecutor {
    ...
    private void handleGive(Player player, String[] args) {
        if (args.length < 2) {
            player.sendMessage("§cUsage: /nexussurvival give <waterbottle|rawwater|gasmask|wand|feralsummon|tntsummon|plaguemobsummon|cure> [player]");
            return;
        }
        switch (args[1].toLowerCase()) {
            ...
            default -> player.sendMessage("§cUnknown item. Options: waterbottle, rawwater, gasmask, wand, feralsummon, tntsummon, plaguemobsummon, cure");
        }
    }
    ...
    private void handleResetMe(Player player) {
        ...
        data.lastProblemMessageAt.clear();
