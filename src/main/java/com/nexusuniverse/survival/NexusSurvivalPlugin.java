...
public class NexusSurvivalPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        // Registered so other Nexus plugins (NexusDreams first) can read a player's current
        // thirst/radiation/hygiene/infection state via Bukkit's ServicesManager + reflection,
        // without ever needing NexusSurvival as a compile-time dependency - the same
        // soft-dependency pattern NexusServerRules already uses for NexusRealms. See
        // api/NexusSurvivalApi.java for the full rationale.
        this.api = new NexusSurvivalApi(playerDataManager);
        ...
        this.diseaseManager = new DiseaseManager(playerDataManager, config, seasonBridge);
        ...
        this.climateManager = new ClimateManager(playerDataManager, config, seasonBridge, sunCapItem);
