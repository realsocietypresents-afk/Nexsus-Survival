...
public class RadiationManager {
    ...
    public RadiationManager(PlayerDataManager playerData, RadiationItems radiationItems, MoralityMessengerBridge messenger) {
    ...
    public void tick(Player player) {
        ...
        if (inZone && !protectedByMask) {
            ...
            if (d.radOxygen <= 0 && d.radTickCounter % SUFFOCATE_DAMAGE_INTERVAL_SECONDS == 0) {
                player.damage(1.0);
                messenger.announce(player, "survival.radiation_suffocation",
                        "You're out of rad-oxygen and suffocating in a radiation zone.",
                        "Get out of the zone, or wear a Hazmat Mask to stop draining oxygen there in the first place.");
            }
