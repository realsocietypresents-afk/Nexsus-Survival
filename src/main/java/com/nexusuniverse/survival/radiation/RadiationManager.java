...
public class RadiationManager {
    ...
    public RadiationManager(PlayerDataManager playerData, RadiationItems radiationItems, ProblemAdvisor advisor) {
    ...
    public void tick(Player player) {
        ...
        if (inZone && !protectedByMask) {
            ...
            if (d.radOxygen <= 0 && d.radTickCounter % SUFFOCATE_DAMAGE_INTERVAL_SECONDS == 0) {
                player.damage(1.0);
                advisor.notify(player, d, "radiation",
                        "§4§lRadiation is suffocating you! §cYour Rad-O2 is empty inside a radiation zone.",
                        "§7Solution: leave the zone immediately, or put on a Hazmat Mask to stop the drain entirely.");
            }
