...
public class ClimateManager {
    ...
    public ClimateManager(PlayerDataManager playerData, NexusSurvivalConfig config, SeasonBridge seasonBridge,
                           SunCapItem sunCapItem, MoralityMessengerBridge messenger) {
    ...
    private void applyHypothermiaEffects(Player player, SurvivalPlayerData d) {
        ...
        if (d.hypothermiaStage >= 4 && random.nextDouble() < 0.34) { // roughly once every 3s at max stage, not every single tick
            player.damage(1.0);
            messenger.announce(player, "survival.hypothermia",
                    "You're freezing and taking damage from hypothermia.",
                    "Get out of the storm and into shelter, or wear warm gear, until you've warmed back up.");
        }
    }
    ...
    private void applyHeatEffects(Player player, SurvivalPlayerData d) {
        ...
        if (d.heatStage >= 4 && random.nextDouble() < 0.34) { // roughly once every 3s at max stage, same odds as hypothermia's stage 4
            player.damage(1.0);
            messenger.announce(player, "survival.heat_stroke",
                    "You're suffering heat stroke and taking damage from the sun.",
                    "Get into shade or indoors, wait out the storm, or wear a Sun Cap to stop heat buildup completely.");
        }
    }
