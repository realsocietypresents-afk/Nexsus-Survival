...
public class DiseaseManager {
    ...
    public DiseaseManager(PlayerDataManager playerData, NexusSurvivalConfig config,
                           com.nexusuniverse.survival.seasons.SeasonBridge seasonBridge, ProblemAdvisor advisor) {
    ...
    /** No known cause to report -- prefer the overload below with a cause whenever the call site actually knows one, so the player isn't left guessing why they got sick. */
    public void infect(Player player, Disease disease) {
        infect(player, disease, null);
    }

    /**
     * @param cause a short, lowercase phrase describing what caused this ("the biome you're
     *              standing in", "your filthy gear", "the untreated water you drank", ...), or
     *              null if genuinely unknown -- folded into the "you feel unwell" message so
     *              getting sick is never a mystery, on top of the existing description/title/sound.
     *              Also appends a concrete "how to cure this" solution line, using the exact
     *              ingredients DiseaseCureRecipes actually registered for this disease.
     */
    public void infect(Player player, Disease disease, String cause) {
        ...
        player.sendMessage("§4You feel unwell... §c[" + disease.getDisplayName() + "]"
                + (cause != null ? " §7-- from " + cause + "." : ""));
        player.sendMessage(disease.getDescription());
        player.sendMessage("§7Solution: craft a Cure: " + disease.getDisplayName() + " potion (Water Bottle + "
                + DiseaseCureRecipes.ingredientHint(disease) + ") and drink it before this gets worse.");
    ...
    public void infectRandom(Player player) {
        infectRandom(player, null);
    }

    public void infectRandom(Player player, String cause) {
        Disease[] all = Disease.values();
        infect(player, all[random.nextInt(all.length)], cause);
    }
    ...
    public boolean cure(Player player, Disease disease) {
        ...
        advisor.clear(d, "disease-critical");
        ...
        for (Player player : onlinePlayers) {
            ...
            for (PlagueSite site : plagueSites) {
                ...
                if (random.nextDouble() < site.disease().contagionChance() * config.diseaseChanceMultiplier()) {
                    infect(player, site.disease(), "the remains nearby");
                }
    ...
    private void applySymptoms(Player player, SurvivalPlayerData d) {
        ...
        if (d.infectionSeverity >= MAX_SEVERITY) {
            ...
            advisor.notify(player, d, "disease-critical",
                    "§4§lYour " + d.infection.getDisplayName() + " has turned CRITICAL! §cIt's now dealing real damage.",
                    "§7Solution: craft and drink a Cure: " + d.infection.getDisplayName() + " potion (Water Bottle + "
                            + DiseaseCureRecipes.ingredientHint(d.infection) + ") immediately.");
        } else {
    ...
    private void trySpread(Player source, Disease disease) {
        ...
        for (Entity nearby : source.getNearbyEntities(radius, radius, radius)) {
            ...
            if (random.nextDouble() < disease.contagionChance() * config.diseaseChanceMultiplier()) {
                infect(target, disease, "close contact with " + source.getName());
    ...
    private void tryInfectFromBiome(Player player, SurvivalPlayerData d) {
        ...
        for (Disease disease : Disease.values()) {
            ...
            if (random.nextDouble() < config.getDouble("disease.biome-exposure-chance", 0.08) * seasonalMultiplier() * config.diseaseChanceMultiplier()) {
                infect(player, disease, "the biome you're standing in");
            }
    ...
    public void tryInfectFromBlock(Player player, Material material) {
        ...
        for (Disease disease : Disease.values()) {
            ...
            if (random.nextDouble() < config.getDouble("disease.block-contact-chance", 0.10) * seasonalMultiplier() * config.diseaseChanceMultiplier()) {
                infect(player, disease, "something you just touched");
            }
            return;
        }
    }
    ...
    public void tryInfectFromMob(Player player, EntityType mobType) {
        ...
        for (Disease disease : Disease.values()) {
            ...
            if (random.nextDouble() < config.getDouble("disease.mob-contact-chance", 0.10) * seasonalMultiplier() * config.diseaseChanceMultiplier()) {
                infect(player, disease, "that mob's attack");
            }
            return;
        }
    }
