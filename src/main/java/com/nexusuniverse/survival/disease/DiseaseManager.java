...
public class DiseaseManager {
    ...
    public DiseaseManager(PlayerDataManager playerData, NexusSurvivalConfig config, com.nexusuniverse.survival.seasons.SeasonBridge seasonBridge, MoralityMessengerBridge messenger) {
    ...
    private void applySymptoms(Player player, SurvivalPlayerData d) {
        ...
        if (d.infectionSeverity >= MAX_SEVERITY) {
            ...
            messenger.announce(player, "survival.disease_critical",
                    "Your " + d.infection.getDisplayName() + " has reached CRITICAL severity and is dealing real damage.",
                    "Get a cure for it as fast as possible -- /nexussurvival status shows what you're infected with.");
        } else {
