...
public final class DiseaseCureRecipes {
    ...
    // populated by register() below, on plugin enable, before anything could need to look a hint
    // up -- ingredientHint() reads from this rather than keeping its own separate list, so the
    // in-game "here's how to cure it" message text can never drift out of sync with the actual
    // crafting recipe.
    private static final Map<Disease, String> INGREDIENT_HINTS = new EnumMap<>(Disease.class);
    ...
    private static void register(Plugin plugin, DiseaseItems diseaseItems, ThirstItems thirstItems,
        ...
        INGREDIENT_HINTS.put(disease, prettyName(ingredientA) + " + " + prettyName(ingredientB));
    ...
    /**
     * Human-readable "Item A + Item B" for that disease's cure recipe (on top of the Water Bottle
     * every cure also needs) -- used by DiseaseManager's infection/critical-damage messages so the
     * in-game solution text is always concrete ("craft X + Y"), never a vague "go find its cure."
     */
    public static String ingredientHint(Disease disease) {
        return INGREDIENT_HINTS.getOrDefault(disease, "its matching cure ingredients");
    }
